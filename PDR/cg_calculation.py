...
t_c = 0.12
W_fuel = 923 #lbs, fuel weight
fuel_density = 6.676 #lbs/gal, fuel density
V_t = W_fuel*(1/fuel_density) #gal, total fuel volume
print(f'Total Fuel Volume = {V_t}')
V_i = V_t/N_t #gal, integral tanks volume
W_dg = 9923.42 #lb, design gross weight
W_en = 331 #lb, engine weight for each engine
W_fw = W_fuel #lb, weight of fuel in wing
...
# Fuselage weight
W_fuselage = 0.052*S_f**1.086*(N_z*W_dg)**0.177*L_t**(-0.051)**(L/D)**(-0.072)*q**0.241+W_press
...
W_installedengine = 2.575*W_en**0.922*N_en + W_en 
# Fuel system weight
...
takeoff_weight = W_wing+W_ht+W_vt+W_fuselage+W_mainlandinggear+W_taillandinggear+W_installedengine+W_fuelsystem+W_flightcontrols+W_hydraulics+W_avionics+W_electrical+W_furnishings+W_battery+W_payload+W_fuel
print(f'Takeoff Weight = {takeoff_weight}')
empty_weight = W_wing+W_ht+W_vt+W_fuselage+W_mainlandinggear+W_taillandinggear+W_installedengine+W_fuelsystem+W_flightcontrols+W_hydraulics+W_avionics+W_electrical+W_furnishings+W_battery
print(f'Empty Weight = {empty_weight}')