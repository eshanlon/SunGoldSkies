...
def weight_estimation(Wcrew, Wpayload, Wo, batt_se, batt_eff, m_fuel, P, P_DP, S, S_DP, max_interations = 100):
    ...
    iterationcount = []
    v = 236.319 # ft/s
    n_f = 1 # percent of power used for turboprop
    ...
    while delta > err and iteration < max_interations:
        ...
        P_aircraftf = v * (Wo / (550*LD)) * n_f# [ft-lbf/s]
        ...
        W5_Wo = New_W5_Wo
        Wf_Wo = (1- W5_Wo) * 1.10
        New_Wo = (Wcrew + Wpayload) / (1 - We_Wo - ((Wf_Wo))) #+ (n_f*((m_batt*g)/ Wo) * (1 + E01_EC + E12_EC + E23_EC + E34_EC + E45_EC))))
...
Wpayload = 5700 # lbm * g / 32.17 lbm = lbf
...
P = 800
P_DP = 800
converged_weight = weight_estimation(Wcrew, Wpayload, Wo, batt_se, batt_eff, m_fuel, P, P_DP, S, S_DP)
